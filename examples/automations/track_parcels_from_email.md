# Automatic parcel tracking from e-mail (IMAP → UPS)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the UPS tracking code, and register it with `ups.track_parcel` — fully automatic, no extra custom component required.

UPS is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `ups.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ ups.track_parcel
                                     │
...
- This integration, with the `ups.track_parcel` action available (field `tracking_code`).
...
The regex matches the standard `1Z` package format — two letters, then 16 more letters/digits (18 characters total):

```
(?<![0-9A-Za-z])1Z[0-9A-Za-z]{16}(?![0-9A-Za-z])
```

UPS also tracks Mail Innovations, InfoNotice and reference numbers through
the same field, but those have no fixed, greppable shape — anchoring on
`1Z…` catches the common consumer case reliably without matching every order
number in the mail; the AI fallback is what picks up the rest. The
integration itself accepts any non-empty code, so a tracking number the
regex misses still works fine typed in manually or found by the AI path.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@upsshipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: 1Z999AA10123456784","initial":true,"folder":"INBOX","username":"test"}'
```

Then `ups.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
