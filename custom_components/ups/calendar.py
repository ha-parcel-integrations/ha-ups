"""Calendar platform for the UPS parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the UPS deliveries calendar from a config entry."""
    async_add_entities([UPSDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class UPSDeliveriesCalendar(CoordinatorEntity[UPSCoordinator], CalendarEntity):
    """A read-only calendar of expected UPS deliveries.
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "UPS parcel")
            description_parts = [
